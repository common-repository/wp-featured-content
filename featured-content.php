...
<div class="wrap" id="featured-content-order">
	...
	function save_non_item_areas( $id, $post ) {
		...
		foreach ( (array) $terms as $term ) {
			...
			if ( empty( $tax_posts->posts ) ) {
				...
				$sql = $wpdb->prepare(
					"UPDATE $wpdb->posts SET post_status = %s WHERE ID IN (" . join( ',', $tax_posts->posts ) . ")",
					$post->post_status
				);
				$wpdb->query( $sql );
		...
		if ( ! empty( $orpan_term_ids ) ) {
			...
			$post_sql = sprintf( "DELETE FROM $wpdb->posts WHERE ID IN (%s)", join( ',', wp_list_pluck( $orphan->posts, 'ID' ) ) );
			$wpdb->query( $post_sql );
			$orphan_ids = wp_list_pluck( $orphan->posts, 'post_parent' );
			$relationship_sql = sprintf(
			...
			array_map( 'clean_post_cache', $orphan_ids );
	...
	function transition_post_status( $new_status, $old_status, $post ) {
		...
		$sql = $wpdb->prepare(
			"UPDATE $wpdb->posts SET post_status = %s WHERE ID IN (" . join( ',', $post_ids ) . ')',
			$new_status
		);
